// File generated by the BNF Converter (bnfc 2.9.4).

package sample.Absyn;

public class Declaration  extends Dec {
  public final String ident_;
  public final Type type_;
  public Declaration(String p1, Type p2) { ident_ = p1; type_ = p2; }

  public <R,A> R accept(sample.Absyn.Dec.Visitor<R,A> v, A arg) { return v.visit(this, arg); }

  public boolean equals(java.lang.Object o) {
    if (this == o) return true;
    if (o instanceof sample.Absyn.Declaration) {
      sample.Absyn.Declaration x = (sample.Absyn.Declaration)o;
      return this.ident_.equals(x.ident_) && this.type_.equals(x.type_);
    }
    return false;
  }

  public int hashCode() {
    return 37*(this.ident_.hashCode())+this.type_.hashCode();
  }


}
